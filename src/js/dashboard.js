...
function getCommentUi(avatar, username, commentText, date) {
    ...
    if (!found)
        fetch('http://localhost:3000/comments/' + post_id)
            ...
            .then((data) => {
                ...
                var comment = getCommentUi(data[0].avatar, data[0].username.slice(1, -1), data[0].comment_text.slice(1, -1), data[0].date);
                $(post).append(comment);
                if (data.length > 1) {
                    for (var i = 1; i < 3 && i < data.length; i++) {
                        comment = getCommentUi(data[i].avatar, data[i].username.slice(1, -1), data[i].comment_text.slice(1, -1), data[i].date);
                        $(post).append(comment);
                    }
...
function submitComment(post_id, post_i) {
    ...
    var data = { userid: userid, commentText: inputval, postId: post_id, noComments: no_comments }
    fetch('http://localhost:3000/addcomment', {
        ...
        .then((data) => {
            ...
            if (data.success == true) {
                ...
                var commentui = getCommentUi(null, "username", inputval, "now")
...
function share(post_id) {
    ...
    fetch('http://localhost:3000/share/' + post_id + '/' + userid + '/' + no_shares)
        .then((response) => response.json())
        .then((data) => {
            console.log(data);
            if (data.success == true) {
